...
# Input original points and get the rotated points
...
# Input original points and get the translate points
...
# Calculate the distance between eye and nose (fix distance)
cal_fixdist = function(points){
  apply(points[c(1,10),],2,mean)[2] - points[44,][2]
}

# Input original points and get the zoomed points
cal_zoom = function(points, rate){
  points*unlist(rate)
}

# Do points and images preprocessing
move_images = function(points, img, center_x = 500, center_y = 375, fixdist = -170, single = c(35,36,37,38,44,52,56,59,62), double = data.frame(x1 = c(1,2,3,4,5,6,7,8,9,19,20,21,22,23,24,25,26,39,40,41,42,43,50,51,57,58,63), x2 = c(10,15,14,13,12,11,18,17,16,31,30,29,28,27,34,33,32,49,48,47,46,45,54,53,55,60,61))){
...
# Do move_images for all points
change_image = function(indices = c(1,2), train_dir = "../data/train_set/", fiducial = fiducial_pt_list, single = c(35,36,37,38,44,52,56,59,62), double = data.frame(x1 = c(1,2,3,4,5,6,7,8,9,19,20,21,22,23,24,25,26,39,40,41,42,43,50,51,57,58,63), x2 = c(10,15,14,13,12,11,18,17,16,31,30,29,28,27,34,33,32,49,48,47,46,45,54,53,55,60,61))){
  ...
  fiducial_pt_list_sub = fiducial[indices]
...
# Do points preprocessing
move_points = function(points, center_x = 500, center_y = 375, fixdist = -170, single = c(35,36,37,38,44,52,56,59,62), double = data.frame(x1 = c(1,2,3,4,5,6,7,8,9,19,20,21,22,23,24,25,26,39,40,41,42,43,50,51,57,58,63), x2 = c(10,15,14,13,12,11,18,17,16,31,30,29,28,27,34,33,32,49,48,47,46,45,54,53,55,60,61))){
  pos_single = data.frame(x = points[single,1], y = points[single,2])
  pos_double=data.frame(x = 0.5*points[double[,1],1]+0.5*points[double[,2],1],
                        y = 0.5*points[double[,1],2]+0.5*points[double[,2],2])
  pos=rbind(pos_single,pos_double)
...
# Do move_points for all points
change_points = function(indices = c(1,2), train_dir = "../data/train_set/", fiducial = fiducial_pt_list, single = c(35,36,37,38,44,52,56,59,62), double = data.frame(x1 = c(1,2,3,4,5,6,7,8,9,19,20,21,22,23,24,25,26,39,40,41,42,43,50,51,57,58,63), x2 = c(10,15,14,13,12,11,18,17,16,31,30,29,28,27,34,33,32,49,48,47,46,45,54,53,55,60,61))){
  train_image_dir <- paste(train_dir, "images/", sep="")
  train_pt_dir <- paste(train_dir,  "points/", sep="")
  train_label_path <- paste(train_dir, "label.csv", sep="") 
