...
| gbm_colored | 48.8% | 45.2s | 188s | 0.04s | 68.6s |
...
All test time = Data preprocessing time + Feature extraction time + Test prediction time
...
+ feature: function that takes the updated feature that were selected by ourselves
  + feature function has a boolean input which indicate if we want to take color features or not
  + color feature includes the darker folds color on the face
...
+ we also saved the features into the output folder
...
In this step, we will construct the train model using GBM, SVM, and XGB, respectively using baseline features, improved features and improved features with color features to get the training model, then saved these model to the output folder.
...
For each model we constructed, we use that model on the test set and give a confusion matrix and predicting accuracy repectively. 
