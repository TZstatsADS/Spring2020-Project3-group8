...
```{r runcv, eval=F}
source("../xjx_lib/cross_validation_knn.R")
if(run.cv){
...
```{r gbm_train}
tm_train  = NA
tm_train <- system.time(fit_train <- gbm_train(dat_train))
save(fit_train, file="../output/fit_train_gbm.RData")
```

### Step 5: Run test on test images
```{r test}
source("../xjx_lib/test_knn.R")
tm_test=NA
if(run.test){
...
gbm_model = gbm_train(dat_train)[[1]]
pred_gbm = gbm_test(gbm_model, dat_test)[[1]]
pred_gbm_class = apply(pred_gbm,1,which.max)
confusionMatrix(pred_gbm_class, dat_test$emotion_idx)
```
