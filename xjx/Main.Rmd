...
all training time = model training time + feature extraction time + data preprocessing time
...
We extract color features and use the svm classifier as our improved model. Since there is no other results affect our judgment of in-class tests. We predict the final accuracy of in-class test to be 58%
...
```{r}
#using the baseline features to train the gbm model, 
if(run.baseline){
  load("../output/feature_train_baseline.RData")
  if(run.train.gbm){
    source("../xjx_lib/train_gbm.R")
    gbm_result_train_baseline = gbm_train(dat_train_baseline, ntrees = 200, bag.fraction = 0.8, shrinkage = 0.1, cv.folds = 3)
...
if(run.train.gbm){
  ...
  gbm_result_train = gbm_train(dat_train, ntrees = 200, bag.fraction = 0.8, shrinkage = 0.1, cv.folds = 3)
...
if(run.train.gbm){
  ...
  gbm_result_train_color = gbm_train(dat_train_color, ntrees = 200, bag.fraction = 0.8, shrinkage = 0.1, cv.folds = 3)
